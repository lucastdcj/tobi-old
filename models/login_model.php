...
class LoginModel {
  ...
  public function login() {
    ...
    if (password_verify($_POST['user_password'], $result->user_password_hash)) {
      ...
      $sql = "SELECT problem_id FROM problem_user WHERE user_id = :user_id";
      $sth = $this->db->prepare($sql);
      $sth->execute(array(
        ':user_id' => $result->user_id        
      ));
      
      $problems_solved = $sth->fetchAll();      
      ...
      Session::set('user_problems', $problems_array);
      Session::set('section_id', $result->section_id);      
  ...
  public function registerNewUser() {
    ...
    if (!$this->checkCaptcha()) {
      ...
      $_SESSION["feedback_negative"][] = FEEDBACK_EMAIL_DOES_NOT_FIT_PATTERN;
    } elseif (!empty($_POST['user_name']) AND strlen($_POST['user_name']) <= 64 AND strlen($_POST['user_name']) >= 2 AND preg_match('/^[a-z\d]{2,64}$/i', $_POST['user_name']) AND !empty($_POST['user_email']) AND strlen($_POST['user_email']) <= 64 AND filter_var($_POST['user_email'], FILTER_VALIDATE_EMAIL) AND !empty($_POST['user_password_new']) AND !empty($_POST['user_password_repeat']) AND ($_POST['user_password_new'] === $_POST['user_password_repeat'])) {
